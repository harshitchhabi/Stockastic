...
| Filling the server with junk accounts, or one person with two accounts | Optional Google sign-in (verified emails, optional domain limit), an optional list of approved emails (only those people can register, one account per mailbox), an optional event code for registration (organisers can change it live), a cap on accounts (`MAX_ACCOUNTS`), organisers can close registration, and one mailbox is one person (case, `+tag` and Gmail dots are ignored) |
